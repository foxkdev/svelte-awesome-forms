export class Form {
  inputs: object;
  errors: Array<{ input: string; type: string }>;
  validators: object;
  onChange: Function;
  constructor(options) {
  ...
  validate(forceValidate = true) {
    return Object.keys(this.inputs).every((input) =>
      this.validateInput(input, forceValidate),
    );
  }
  validateInput(input, forceValidate = true) {
    ...
    return this.inputs[input].validators.every((validator) => {
      ...
      if (result !== true) {
        ...
        if (
          !this.errors.find(
            (error) => error.input === input && error.type === result,
          )
        ) {
      ...
      if (this[input].errors.includes(result)) {
        this[input].errors = this[input].errors.filter(
          (error) => error !== result,
        );
      }
      return true;
